...
[task4](task4/)目前完成最低限度的17个接口的编写，并且完成所有的注意事项

其实还是有一些需要注意的点的，比如说我的双token认证实现的其实有问题，我现在是强行把access-token转换为hertz默认的auth-token，然后不管另一个token，这点会在task5中修正

还有一些问题，比如说docker部分其实就是纯使用AI辅助的，然后也把一些乱七八糟的东西打包进去了，还有就是一直在抄tiktok-demo的实现（其实中后期就没怎么参考了）
